...
def run():
    ...
    # 1. Set Nest to sleep mode (ECO 62-80°F for rest of house)
    try:
        ...
        actions.append(f"Nest sleep failed: {str(e)[:30]}")

    # 2. Set Sensibo to sleep mode (active 66°F for bedroom)
    try:
        ...
        api_start = time.time()
        sensibo.set_sleep_mode()  # Intent-based API - idempotent, config-driven (66°F)
        duration_ms = int((time.time() - api_start) * 1000)
        ...
        actions.append(f"Sensibo sleep failed: {str(e)[:30]}")
